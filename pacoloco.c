...
struct statistics {
    ...
    // e.g. db check request might fail but we still redirect it upstream hoping that it is some kind of transient
    // error.
...
static void send_check_request_to_peer(struct incoming_req *incoming_req, struct peer *peer) {
    ...
    debug("[%d] send check request to peer [%d] %s:%d/%s/%s", incoming_req->client->fd, peer->fd, peer->host,
          peer->port, prefix, filename);
    ...
    if (events & EPOLLIN) {
        ...
        while (true) {
            ...
            while (bytes_left > 0) {
                ...
                parsed = phr_parse_request(buf_start, bytes_left, &method, &method_len, &path, &path_len,
                                           &minor_version, headers, &headers_num, bytes_left_last);
